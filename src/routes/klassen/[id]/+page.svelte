<!-- Entfernt die localStorage-Logik und verwendet nur noch das Standard-Formular -->
<script lang="ts">
	...
	let isUploading = false; // Wird nur noch für Lehrer relevant sein
	...
	async function handleUpload(event: SubmitEvent) {
		// Diese Funktion bleibt gleich, wird aber nur angezeigt, wenn user NICHT anonym ist
		if (!supabase || !session || !user || user.is_anonymous) { form = { error: true, message: 'Nur Lehrpersonen können hochladen.' }; return; } // Sicherstellen, dass anonyme User hier nicht hinkommen
...
<div class="min-h-screen bg-gray-50 text-gray-800 font-sans selection:bg-pink-500 selection:text-white">
	<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
		<!-- Header: Link zum Dashboard nur für eingeloggte Lehrer anzeigen -->
		{#if classData}
			<div class="relative bg-white p-6 sm:p-8 rounded-xl shadow-xl mb-10 overflow-hidden">
				...
				<div class="relative z-10">
					...
					{:else}
						<!-- Anonyme User sehen keinen "Zurück"-Link oder einen Link zur Startseite -->
					...
					<!-- Klassencode nur für Lehrer anzeigen -->
					{#if user && !user.is_anonymous}
						...
						{#each songs as song, i (song.id)}
							<div class="bg-white p-4 rounded-xl shadow-lg flex flex-col gap-3 border-l-4 border-pink-500 transform hover:-translate-y-1 hover:shadow-xl transition-all duration-200 ease-in-out">
								...
								<div class="flex items-center gap-4">
									...
									<!-- Löschen-Button nur für Lehrer -->
									{#if user && !user.is_anonymous && user.id === classData?.owner_id}
								...
								<!-- Bewertung: Immer das Standard-Formular verwenden -->
								<div class="flex justify-between items-center mt-2 pt-3 border-t border-gray-100">
									...
									<!-- Dieses Formular funktioniert jetzt für Lehrer UND anonyme Schüler -->

						...
						{#each data.songs as song, i (song.id)}
							<div class="bg-white p-4 rounded-xl shadow-lg flex flex-col gap-3 border-l-4 border-pink-500 transform hover:-translate-y-1 hover:shadow-xl transition-all duration-200 ease-in-out">
								...
								<div class="flex items-center gap-4">
									...
									<div class="flex-grow min-w-0"> <!-- Korrigierter Kommentar -->
			...
			<div class="lg:col-span-1">
				<div class="bg-white p-6 rounded-xl shadow-lg sticky top-10">
					...
					<form id="upload-form" on:submit|preventDefault={handleUpload} class="space-y-4">
						...
						<button type="submit" disabled={isUploading} class="w-full bg-pink-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-pink-700 transition-colors shadow-lg disabled:bg-gray-400 disabled:cursor-not-allowed">
							{#if isUploading}
...
<style>
	/* Fügt benutzerdefinierte Stile hinzu, die Tailwind überschreiben oder ergänzen können */
	button:disabled {
		cursor: not-allowed;
	}
	audio {
		display: none; /* Versteckt das Standard-Audio-Element */
	}
</style>
